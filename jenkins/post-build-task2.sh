...
curl -X "POST" "https://api.github.com/repos/owaiskazi19/OpenSearch/issues?state=all" \
...
then
    ...
    IFS=`echo -en "\n\b"` && for i in `curl -s https://api.github.com/repos/owaiskazi19/OpenSearch/issues -H "Authorization: token $GITHUB_PASSWORD_CREDENTIAL" | jq -r .[].title`;
    do 
        echo $i
